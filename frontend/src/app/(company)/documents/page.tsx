...
useEffect(() => {
  ...
  return (
    <div className="container mx-auto p-4 lg:p-8">
      <div className="mb-4 text-sm">
        ...
        <Link href="/documents" className="text-gray-500 hover:underline">{" "}Documents</Link>
      </div>
  ...
  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
    {filteredDocuments.map((doc, index) => {
      const uniqueKey = doc._id; // Use the document's unique ID directly
      const isLastDocument = filteredDocuments.length === index + 1;

      return (
        <Link
          key={uniqueKey} // Use the unique ID as the key
